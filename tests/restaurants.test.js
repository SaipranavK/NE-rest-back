...
describe("NENT - Tech Trainee code challenge Integration tests\n-----------------------", () => {
    ...
    it("Initiate CRUD tests", done => {
        ...
        test("POST /api/v1/restaurants/ should return errors from payload validation on request with invalid payload", assert => {
            const restaurant = {"opening_hours": [], "address": "", "phone_number": "08-641 20 77", "location": {"lat": 59.31781179999999, "lng": 18.0701277}, "icon": "", "name": "Tamarindo", "price_level": 1, "rating": 4.4, "google_maps_url": "https://maps.google.com/?cid=9369167126300605621", "website": "http://www.tamarindo.se/", "photo": "https://cdn.pixabay.com/photo/2016/11/18/22/21/architecture-1837150_1280.jpg"}
            request(server)
            .post("/api/v1/restaurants/")
            .send(restaurant)
            .expect(422)
            ...
            .end((err, res) => {
                if (err) return assert.fail(JSON.stringify(res));
                assert.pass("Document not added due to invalid payload\n-------------------------");
                assert.end();
            });
        });
